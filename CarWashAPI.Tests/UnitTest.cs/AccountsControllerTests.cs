...
namespace CarWashAPI.Tests.UnitTest.cs
{
    ...
    public class AccountsControllerTests : BaseTests
    {
        ...
        public async Task UserCanNotLogin()
        {
            ...
            var userInfo = new UserInfo() { UserName = "Mmilos", Password = "badPassword" };
        ...
        public async Task UserCanLogin()
        {
            ...
            var controller = BuildAccountsController(databaseName);
            var userInfo = new UserInfo() { UserName = "Mmilos", Password = "ValueShore3!" };
            var response = await controller.Login(userInfo);
        ...
        private async Task CreateAnUser(string databaseName)
        {
            ...
            var userInfo = new UserInfo() { UserName ="Mmilos",IsOwner = true, EmailAddress = "example@hotmail.com", Password = "ValueShore3!", Address = "Lipetska12", FirstName = "Milos", LastName = "Mijatovic" };
            await accountsController.CreateUser(userInfo);
        }
        
        private AccountController BuildAccountsController(string databaseName)
        {
            ...
            {
                {"JWT:key", "SAMJOISJIFJDSFAEJFAPSJDPOA" }
            };
            ...
            return new AccountController(context,mapper,userManager,configuration,signInManager);
        }
